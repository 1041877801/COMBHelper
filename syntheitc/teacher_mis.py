...
def train():
    ...
    logger.info('GNN backbone is GraphSAGE')
    ...
    for epoch in range(epochs):
        ...
        if acc_val > acc_best:
            acc_best = acc_val
            # torch.save({'model': model.encoder.state_dict(), 'weights': updated_weights}, config['teacher']['ckpt_path']['MIS'])
            logger.info('Acc_best is updated to {:.4f}. Model checkpoint is saved to {}.'.format(acc_best, config['teacher']['ckpt_path']['MIS']))
