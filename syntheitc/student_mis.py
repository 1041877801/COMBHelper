...
from models import GCN1, GCN2, StudentModel
...
def train():
    ...
    for epoch in range(epochs):
        ...
        if acc_val > acc_best:
            acc_best = acc_val
            # torch.save({'model': model.encoder_s.state_dict(), 'weights': updated_weights}, config['student']['ckpt_path']['MIS'])
            logger.info('Acc_best is updated to {:.4f}. Model checkpoint is saved to {}'.format(acc_best, config['student']['ckpt_path']['MIS']))
